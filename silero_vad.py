...
_DEFAULT_MODEL_PATH = _DEFAULT_MODEL_DIR / "silero_vad.onnx"
...
try:
    ...
    logger.error("In order to use Silero VAD, you need to install `onnxruntime`.")
    raise Exception(f"Missing module(s): {e}")

def resolve_model_path(model_path: str | Path | None = None) -> str:
    """Resolve the ONNX model path, downloading it if needed.

    Priority order:
    1. Explicit path passed by the caller.
    2. Repo-local models/silero_vad.onnx.
    3. Download from the official Silero VAD repository.
    """
    candidate = Path(model_path).expanduser().resolve() if model_path else _DEFAULT_MODEL_PATH
    if candidate.is_file():
        return str(candidate)

    candidate.parent.mkdir(parents=True, exist_ok=True)
    logger.info("Downloading Silero VAD model to %s", candidate)
    urlretrieve(_DEFAULT_MODEL_URL, candidate)
    return str(candidate)
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            model_path: Optional local path to a Silero ONNX model file.
        """
        super().__init__(sample_rate=sample_rate, params=params)

        logger.debug("Loading Silero VAD model...")

        model_file_path = resolve_model_path(model_path)

        self._model = SileroOnnxModel(model_file_path, force_onnx_cpu=True)
        logger.info("Silero VAD model loaded from %s", model_file_path)
